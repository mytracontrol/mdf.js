...
import { ArchiverManager } from '..';
import { Factory } from './Factory';
import { Port } from './Port';
import { Config } from './types';

const DEFAULT_CONFIG: Config = {
  workingFolderPath: './data/open',
  archiveFolderPath: './data/closed',
  fileEncoding: 'utf-8',
  createFolders: true,
...
describe('#Port #jsonl-archiver', () => {
  ...
  describe('#Happy path', () => {
    it('Should create provider using the factory instance with default configuration', () => {
      ...
      expect(provider.client).toBeInstanceOf(ArchiverManager);
    ...
    it('Should create provider using the factory instance with a configuration', () => {
      ...
      const provider = Factory.create({
        ...
        config: {
          workingFolderPath: './custom/open',
          archiveFolderPath: './custom/closed',
          fileEncoding: 'ascii',
          createFolders: false,
  ...
  describe('#Sad path', () => {
    it('Should emit unhealthy event if there is an error in a jsonl file storage operation', done => {
      const port = new Port(
        { ...DEFAULT_CONFIG, retryOptions: { attempts: 1 } },
      ...
      port
        .start()
        .then(() => {
          return port.client.append({ message: 'Hello' }, 'file1');
        })
        .then(() => {
    ...
    it('Should emit healthy event if there is a recovery from previous error in a jsonl file storage operation', async () => {
      const port = new Port(
        { ...DEFAULT_CONFIG, retryOptions: { attempts: 1 } },
        new FakeLogger() as LoggerInstance
      );
      ...
      await port.start();
      try {
        await port.client.append({ message: 'Hello' }, 'file1');
      } catch (error) {}
      await port.client.append({ message: 'Hello' }, 'file1');
      expect(unhealthyHandler).toHaveBeenCalledTimes(1);

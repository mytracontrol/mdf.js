...
export class MyJetPlug extends EventEmitter implements Plugs.Sink.Jet {
  name = 'MyJetPlug';
  componentId = v4();
  shouldReject = 0;
  irresolvable = false;
  ...
  public multi(jobs: Plugs.Sink.JobObject[]): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      if (!jobs) {
        reject(new Crash('Jobs are required'));
      } else {
        process.nextTick(() => {
          if (this.shouldReject > 0) {
            this.shouldReject--;
